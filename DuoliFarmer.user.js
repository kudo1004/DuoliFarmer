...
const initDuoFarmer = async () => {
  const containerHTML = `<div class=_container id=_container><div class=_header><p class=_title>DuoliFarmer</div><div class=_description><p id=_loginStatus>Đây là phiên bản Mini nên mới chỉ được 10 XP mỗi giây buff, nhưng bù lại có tính năng treo máy.</div><div class=_content><div class=_radioButtons><button class=active data-option=10>10 XP/s</button> </div><button class=_startBtn id=_startBtn>Bắt Đầu</button></div><footer class=_footer><a href=https://r.naturelye.com/discord target=_blank>Discord</a> <span id=_version class=_version>v2.0-mini</span></footer></div><button class=_toggleBtn id=_toggleBtn>→</button><div class=_overlay id=_loadingOverlay><div class=_swalModal><h2>DuoliFarmer</h2><p id=_loadingMessage>Liên hệ với Discord của tôi để được hỗ trợ.<div class=_spinner id=_spinner></div><div class=_hint id=_hint>Bạn Không Nên Treo Quá Nhiều Trong Một Ngày. Nếu Tăng Nhanh Quá Mức, Bạn Có Thể Sẽ Bị Cấm Và Đá Khỏi Bảng Xếp Hạng.</div><div class=_xpInfo>+ <span id=_xpAmount>0</span> XP ✨</div><button id=_stopOverlayBtn>Dừng</button></div></div>`;
  ...
  if (!JWT) {
    ...
    document.getElementById("_loginStatus").innerText = "Oops, có vẻ bạn chưa đăng nhập rồi. (Nếu bạn đã đăng nhập hãy tải lại trang này)";
    return;
  }
  ...
  document.getElementById("_loginStatus").innerHTML = `Xin chào <strong>${username}</strong>! <br>Chào mừng bạn đã đến với bình nguyên vô tận DuoliFarmer, chiến thần Farm Xp Duolingo. Nếu bạn cần hỗ trợ, hãy liên hệ Discord của tôi.`;
